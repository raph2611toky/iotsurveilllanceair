...
function chooseWireColor(fromPin, toPin, fallbackColor) {
  ...
  if (text.includes("GND") || text.includes("−") || text.includes("-")) return "#222222";
...
function isVoltagePin(pinName = "") {
  const text = normalizePinName(pinName);
  return (
    text.includes("VCC") ||
    text.includes("5V") ||
    text.includes("3V3") ||
...
function makeWirePath(points) {
  ...
  if (points.length === 2) {
    ...
    const curve = Math.min(90, Math.max(24, Math.sqrt(dx * dx + dy * dy) * 0.16));
    const direction = from.x <= to.x ? 1 : -1;
    const c1 = { x: from.x + direction * Math.max(dx * 0.3, curve), y: from.y };
    const c2 = { x: to.x - direction * Math.max(dx * 0.3, curve), y: to.y };
...
function createFlexiblePoints(from, to) {
  ...
  const curve = Math.min(120, Math.max(38, distance * 0.16));

  return [
    { x: from.x + dx * 0.25, y: from.y + dy * 0.25 - curve },
    { x: from.x + dx * 0.5, y: from.y + dy * 0.5 + curve * 0.25 },
    { x: from.x + dx * 0.75, y: from.y + dy * 0.75 + curve },
  ];
}
...
function rotatePointAroundCenter(item, component, pin) {
  const angle = ((item.rotation || 0) * Math.PI) / 180;
  const centerX = item.x + component.width / 2;
  const centerY = item.y + component.height / 2;
  const localX = pin.x - component.width / 2;
  const localY = pin.y - component.height / 2;
...
function SensorSimulationEffect({ item, running, data, powered }) {
  if ((!running && !powered) || !data) return null;
...
function ComponentInfoCard({ item, component, powered }) {
  return (
    <div className="component-info-card">
      <strong>{component.name}</strong>
      <span>{component.description || "Composant virtuel du simulateur IoT."}</span>
      <em>Type : {component.type}</em>
      <em>Version : {component.version || "Virtuel 1.0"}</em>
      {component.protocol && <em>Protocole : {component.protocol}</em>}
      {component.voltage && <em>Alimentation : {component.voltage}</em>}
      {item.type !== "rpi5" && (
        <b className={powered ? "powered" : ""}>{powered ? "Alimenté" : "Non alimenté"}</b>
      )}
    </div>
  );
}

function ComponentPowerLeds({ type, powered }) {
  if (!powered || type === "rpi5" || type === "breadboard") return null;
  ...
  return (
    <div className="component-power-leds">
...
export default function EditorCanvas({
  ...
  const poweredItemIds = useMemo(() => {
    ...
    const rpiItems = items.filter((item) => item.type === "rpi5" && item.powered);
    if (rpiItems.length === 0) return powered;
    ...
    wires.forEach((wire) => addEdge(makePinKey(wire.from), makePinKey(wire.to)));

    function connectedSet(startKeys) {
    ...
    rpiItems.forEach((rpi) => {
      const component = componentsData[rpi.type];
      component?.pins?.forEach((pin) => {
        const key = `${rpi.id}:${getPinUniqueKey(pin)}`;
        if (isVoltagePin(pin.name)) rpiPowerKeys.push(key);
        if (isGroundPin(pin.name)) rpiGroundKeys.push(key);
      });
    });

    const powerNetwork = connectedSet(rpiPowerKeys);
    const groundNetwork = connectedSet(rpiGroundKeys);
    ...
    items.forEach((item) => {
      ...
      let hasGround = false;
      component.pins.forEach((pin) => {
        const key = `${item.id}:${getPinUniqueKey(pin)}`;
        if (isVoltagePin(pin.name) && powerNetwork.has(key)) hasVoltage = true;
        if (isGroundPin(pin.name) && groundNetwork.has(key)) hasGround = true;
      });

      if (hasVoltage && hasGround) powered.add(item.id);
    });
  ...
  function resolvePin(pinRef) {
    ...
    if (!pin && pinRef.x !== undefined && pinRef.y !== undefined) {
      pin = component.pins.find(
        (currentPin) =>
          currentPin.name === pinRef.pinName &&
          Math.abs(Number(currentPin.x) - Number(pinRef.x - item.x)) < 0.1 &&
          Math.abs(Number(currentPin.y) - Number(pinRef.y - item.y)) < 0.1
      );
    }
  ...
  function startMoveItem(event, itemId) {
    ...
    function onMove(moveEvent) {
      ...
      setItems((previous) =>
        previous.map((item) =>
          item.id === itemId
            ? { ...item, x: current.x + dx, y: current.y + dy }
            : item
        )
      );
    }
  ...
  function rotateItem(itemId, delta) {
    setItems((previous) =>
      previous.map((item) =>
        item.id === itemId
  ...
  function startWireFromPin(item, pin, event) {
    ...
    setDraftWire({ from, to: getCanvasPoint(event) });

    function onMove(moveEvent) {
      const movePoint = getCanvasPoint(moveEvent);
      setDraftWire((previous) => previous ? { ...previous, to: movePoint } : null);
    }
      ...
      setDraftWire((currentDraft) => {
        ...
        setHoveredPin((currentHovered) => {
          ...
          if (!samePin && !targetAlreadyUsed && !sourceAlreadyUsed) {
            ...
            const color = chooseWireColor(currentDraft.from.pinName, to.pinName, to.color);
            const newWire = {
              id: `wire-${Date.now()}`,
  ...
  function startRewireEndpoint(event, wire, endpoint) {
    ...
    setRewiring({ wireId: wire.id, endpoint, fixedPin, movingPin, cursor: movingPin, color: wire.color });

    function onMove(moveEvent) {
      const movePoint = getCanvasPoint(moveEvent);
      setRewiring((previous) => previous ? { ...previous, cursor: movePoint } : null);
    }
      ...
      setRewiring((currentRewire) => {
        ...
        setHoveredPin((currentHovered) => {
          ...
          const pinUsedByOtherWire = wires.some((currentWire) => {
            ...
            return makePinKey(currentWire.from) === targetKey || makePinKey(currentWire.to) === targetKey;
          });
          ...
          if (targetKey !== fixedKey && !pinUsedByOtherWire) {
            setWires((previous) =>
              previous.map((currentWire) => {
                ...
                const newFrom = currentRewire.endpoint === "from" ? currentHovered : currentWire.from;
                const newTo = currentRewire.endpoint === "to" ? currentHovered : currentWire.to;
                return {
                  ...currentWire,
                  from: newFrom,
                  to: newTo,
                  color: chooseWireColor(newFrom.pinName, newTo.pinName, currentWire.color),
                  points: createFlexiblePoints(newFrom, newTo),
                };
              })
            );
          }
  ...
  function handlePinMouseLeave(item, pin) {
    ...
    setHoveredPin((current) => {
      ...
      if (makePinKey(current) === makePinKey(leavingPin)) return null;
      return current;
    });
  }
      ...
      setWires((previous) =>
        previous.map((currentWire) =>
          currentWire.id === wire.id
            ? { ...currentWire, points: initialPoints.map((point) => ({ x: point.x + dx, y: point.y + dy })) }
            : currentWire
        )
      );
    }
  ...
  function startMoveWirePoint(event, wire, pointIndex) {
    ...
    setDraggingPoint({ wireId: wire.id, pointIndex });
    ...
    function onMove(moveEvent) {
      ...
      setWires((previous) =>
        previous.map((currentWire) => {
          ...
          points[pointIndex] = { x: initialPoint.x + dx, y: initialPoint.y + dy };
          return { ...currentWire, points };
        })
      );
    }
  ...
  function deleteSelected(id) {
    ...
    setWires((previous) => previous.filter((wire) => wire.from.itemId !== id && wire.to.itemId !== id));
    setSelectedId(null);
  }
  ...
  function handleCanvasClick(event) {
    setContextMenu(null);
    const isCanvas = event.target === canvasRef.current || event.target.classList.contains("canvas-grid");
    if (isCanvas) setSelectedId(null);
  }
    ...
    >
      ...
      <div className="canvas-layer" style={transformStyle}>
        {items.map((item) => {
          ...
          component.pins?.forEach((pin) => {
            ...
            const usedByThisRewiring =
              ...
              wires.some((wire) => {
                ...
                if (rewiring.endpoint === "from") return makePinKey(wire.from) === pinKey;
                if (rewiring.endpoint === "to") return makePinKey(wire.to) === pinKey;
                return false;
              });
          ...
          const itemPowered = item.type === "rpi5" ? Boolean(item.powered) : poweredItemIds.has(item.id);

          return (
            <div
              key={item.id}
              className={selectedId === item.id ? "placed selected" : "placed"}
              style={{ left: item.x, top: item.y, width: component.width, height: component.height }}
              onMouseDown={(event) => {
            ...
            >
              <ComponentInfoCard item={item} component={component} powered={itemPowered} />
              ...
              <SensorSimulationEffect item={item} running={running} data={itemSensorData} powered={itemPowered} />
              <ComponentPowerLeds type={item.type} powered={itemPowered} />

              <div className="placed-rotator" style={{ transform: `rotate(${item.rotation || 0}deg)` }}>
                <ComponentVisual
                  component={component}
                  disabledPinKeys={disabledPinKeys}
                  onPinMouseDown={(pin, event) => startWireFromPin(item, pin, event)}
                  onPinMouseEnter={(pin) => handlePinMouseEnter(item, pin)}
                  onPinMouseLeave={(pin) => handlePinMouseLeave(item, pin)}
                  running={(running || itemPowered) && (item.type !== "rpi5" || item.powered)}
      ...
      <svg className="wire-svg" style={transformStyle}>
        {wires.map((wire) => {
          ...
          return (
            <g key={wire.id} className={draggingWireId === wire.id ? "wire-group moving" : "wire-group"}>
              <path
                d={path}
                stroke="transparent"
                strokeWidth="34"
              ...
              <circle cx={from.x} cy={from.y} r="8" fill={wire.color} className="wire-endpoint" onMouseDown={(event) => startRewireEndpoint(event, wire, "from")} onContextMenu={(event) => handleWireRightDoubleClick(event, wire.id)} />
              <circle cx={to.x} cy={to.y} r="8" fill={wire.color} className="wire-endpoint" onMouseDown={(event) => startRewireEndpoint(event, wire, "to")} onContextMenu={(event) => handleWireRightDoubleClick(event, wire.id)} />
              ...
              {(wire.points || []).map((point, index) => (
                <circle
                  ...
                  cx={point.x}
                  cy={point.y}
                  r={draggingPoint?.wireId === wire.id && draggingPoint?.pointIndex === index ? 8 : 5}
                  fill="#fffdf5"
                  stroke={wire.color}
                  strokeWidth="2.5"
                  className="wire-point"
                  onMouseDown={(event) => startMoveWirePoint(event, wire, index)}
                  onContextMenu={(event) => {
                    event.preventDefault();
                    event.stopPropagation();
                    setWires((previous) =>
                      previous.map((currentWire) => {
                        if (currentWire.id !== wire.id) return currentWire;
                        return { ...currentWire, points: currentWire.points.filter((_, pointIndex) => pointIndex !== index) };
                      })
                    );
                  }}
                />
        ...
        {draftWire && (
          <g className="wire-preview">
            <path d={makeWirePath([draftWire.from, draftWire.to])} stroke={draftWire.from.color || "#c9a227"} strokeWidth="4" fill="none" strokeLinecap="round" strokeDasharray="8 6" />
            <circle cx={draftWire.from.x} cy={draftWire.from.y} r="5" fill={draftWire.from.color || "#c9a227"} />
            <circle cx={draftWire.to.x} cy={draftWire.to.y} r="5" fill={draftWire.from.color || "#c9a227"} />
          </g>
        )}

        {rewiring && (
          <g className="wire-preview">
            <path d={makeWirePath([rewiring.fixedPin, rewiring.cursor])} stroke={rewiring.color || "#c9a227"} strokeWidth="4" fill="none" strokeLinecap="round" strokeDasharray="8 6" />
            <circle cx={rewiring.fixedPin.x} cy={rewiring.fixedPin.y} r="5" fill={rewiring.color || "#c9a227"} />
            <circle cx={rewiring.cursor.x} cy={rewiring.cursor.y} r="5" fill={rewiring.color || "#c9a227"} />
          </g>
        )}
      </svg>

      {contextMenu && (
        <div className="rpi-context-menu" style={{ left: contextMenu.x, top: contextMenu.y }}>
          <button
            onClick={() => {
